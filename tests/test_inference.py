...
def test_run_text_inference_returns_empty_when_no_masks():
    ...
    mock_processor.return_value = {"original_sizes": torch.tensor([[64, 64]])}
...
def test_run_point_inference_returns_none_for_empty_mask():
    empty_mask = torch.zeros((1, 3, 64, 64), dtype=torch.bool)
    mock_outputs = MagicMock()
    mock_outputs.pred_masks = empty_mask.unsqueeze(0)
    mock_outputs.iou_scores = torch.zeros((1, 1, 3))

    mock_processor = MagicMock()
    mock_processor.return_value = {"original_sizes": torch.tensor([[64, 64]])}
    mock_processor.post_process_masks.return_value = [[empty_mask]]
